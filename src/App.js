...
const MultiItemCalculator = ({ onCalculate, onAddToCart }) => {
  ...
  const calculateAll = async () => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (item.cetCode && item.fobAmount) {
        ...
        console.log(`Calculating item ${i + 1}: ${item.cetCode}`);
    ...
    console.log(`Total calculated: ${calculatedResults.length} items`);
  ...
  const addToCart = () => {
    if (results.length > 0) {
      console.log(`Adding ${results.length} items to cart`);
      onAddToCart(results);
      ...
      {results.length > 0 && (
        <div className="results-summary">
          ...
          <div className="summary-total">
            ...
            <strong>₦{formatNumber(results.reduce((sum, item) => sum + parseFloat(item.result?.total_payable || 0), 0))}</strong>
          </div>
        </div>
      )}
    </div>
